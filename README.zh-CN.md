...
- 首帧到来前显示全屏 loading 层
- 首帧真正渲染后再隐藏 loading，避免“可播但未出画”的假快
- 优先启动播放器，在播放器进入 `STATE_READY` 后再加载房间数据
- 对 `behind live window` 错误做自动恢复
...
- 首帧 loading 层，优化首屏感知
- 将房间数据初始化延后到播放器可播之后
- 只在 `onRenderedFirstFrame()` 后隐藏 loading，保证“看到画面”再去掉遮罩
- 首帧链路埋点，覆盖建链、缓冲、首帧渲染、错误和恢复阶段
- `behind live window` 自动恢复到最新直播位置
...
3. 当播放器进入 `STATE_READY` 后，开始拉取主播信息和历史评论。
4. 当 `onRenderedFirstFrame()` 回调到来后，隐藏 loading。
...
- 先初始化播放器并准备直播流
- 展示 loading 覆盖层
- 在播放器进入 `STATE_READY` 后开始拉取房间数据
- 在 `onRenderedFirstFrame()` 回调后隐藏 loading
- 拉取主播信息和历史评论
- 通过 WebSocket 接收实时评论和在线人数
- 发生 `behind live window` 时自动 seek 到直播默认位置并重新起播
- 评论区始终保持在最后一条
